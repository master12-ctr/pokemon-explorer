// Map for first 151 Pokémon (enough for evolution chains and main list)
const nameToId: Record<string, number> = {
  ...
  machamp: 68, bellsprout: 69, weepinbell: 70, victreebel: 71, tentacool: 72, tentacruel: 73,
...
export const getPokemonIdByName = (name: string): number => {
  return nameToId[name.toLowerCase()] || 1;
};

export const getPokemonImageUrl = (idOrName: string | number): string => {
  const id = typeof idOrName === 'number' ? idOrName : getPokemonIdByName(idOrName);
